...
export function currencyWithName(amount: number, currencyCode: ISO_4217_Code): string | undefined {
    if (typeof amount !== "number") return undefined;
    const curr = currencyMap.get(currencyCode);
    if (!curr) {
        return `${currencyCode.toUpperCase()} ${amount.toFixed(2)}`;
    ...
    const value = formatter.format(amount);
    return normalizeCurrencyOutput(value) + ` (${curr.name})`;
}

export function currencyParts(amount: number, currencyCode: ISO_4217_Code) {
    if (typeof amount !== "number") return undefined;
    const curr = currencyMap.get(currencyCode);
    if (!curr) return undefined;
    if (curr.isNonStandardCurrency) return undefined;
